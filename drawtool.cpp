...
QGraphicsItem*      DrawTool::c_Item = nullptr;
...
void SelectTool::mousePressEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
	...
	//QGraphicsItem* nowItem = scene->itemAt(event->scenePos(),transform);

	//QGraphicsItem* nowItem  = scene->focusItem();

	if (items.count() == 1)
	{
		item = qgraphicsitem_cast<GraphicsItem*>(items.first());
		/*if (DrawTool::c_Item != nullptr)
		{
			item = qgraphicsitem_cast<GraphicsItem*>(DrawTool::c_Item);
		}*/
		//item = qgraphicsitem_cast<GraphicsItem*>(nowItem);
	}
	...
	/*if (nowItem != 0)
	{
		GraphicsItem* itemt = qgraphicsitem_cast<GraphicsItem*>(nowItem);
		if (itemt != 0)
		{
			nDragHandle = itemt->hitTest(event->scenePos());
			if (nDragHandle != SizeHandleRect::None)
				selectMode = size;
			else
				selectMode = move;
	...
	if (selectMode == SelectMode::move && items.count() == 1)
	{
		if (dashRect)
		{
...
void SelectTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
	...
	/*	if (DrawTool::c_Item)
		...
		{
			//如果是改变大小的模块 
			if (nDragHandle != SizeHandleRect::None && selectMode == SelectMode::size)
			{
				...
				if (nullptr != rectItem)
				{
					int  h = rectItem->getHeight();
					int  w = rectItem->getWidth();
					QGraphicsItem::GraphicsItemChange  change = QGraphicsItem::ItemScaleHasChanged;
					QVariant  temp_event;
					temp_event.setValue(QPointF(w,h));
					item->itemChange(change, temp_event);
				}
			}
	...
	if (selectMode == SelectMode::move)
	{
		if (dashRect)
		{
			...
			dashLine->setLine(QLineF(dashLine->mapFromItem(item,item->boundingRect().center()),dashLine->mapFromScene(c_last)));

			scene->update();
		}
	}

	if ((selectMode != SelectMode::size && items.count() > 1) || isGroup)
	{
		scene->mouseEvent(event);
	}
}
...
void RotationTool::mousePressEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
	...
	{
		GraphicsItem* item = qgraphicsitem_cast<GraphicsItem*>(items.first());
		//GraphicsItem* item = qgraphicsitem_cast<GraphicsItem*>(nowItem);
		if (item != 0)
		{
			nDragHandle = item->hitTest(event->scenePos());
			if (nDragHandle != SizeHandleRect::None)
			{
				...
				setCursor(scene, QCursor(QPixmap(":/Resourse/rotation.png")));
			}
			else
			{
...
void RotationTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
	...
	if (items.count() == 1)
	{
		GraphicsItem* item = qgraphicsitem_cast<GraphicsItem*>(items.first());
		//GraphicsItem* item = qgraphicsitem_cast<GraphicsItem*>(nowItem);
		if (item != 0 && nDragHandle != SizeHandleRect::None && selectMode == SelectMode::rotate)
		{
			//将中心点转换为场景坐标点 
			QPointF origin = item->mapToScene(item->boundingRect().center());

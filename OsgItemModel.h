...
public:
	...
	void saveToFileByName(const QString fileName);

	/// print the entire heirarchy on stderr.  Mostly for debugging.
	void  printTree() const;
	/// print a node of the tree and all its children
	void  printNode(osg::ref_ptr<osg::Node> n,const int level) const;
	void insertNode(osg::ref_ptr<osg::Group> parent, osg::ref_ptr<osg::Node> newChild, int childPositionInParent,
		int row);
...
private:
	...
	QModelIndex modelIndexFromNode(osg::ref_ptr<osg::Node> ptr, int column)const ;
